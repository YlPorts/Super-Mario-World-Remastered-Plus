...
@export var caption := "A"
@export var face_button := true
@export var button_color := Color(0.85, 0.15, 0.18, 0.92)
...
func _ready() -> void:
	...
	caption_label.add_theme_color_override("font_outline_color", Color(0.08, 0.08, 0.12, 1.0))
...
func refresh_visual() -> void:
	...
	caption_label.modulate = Color(1.0, 0.95, 0.35, 1.0) if edit_mode else Color.WHITE
	queue_redraw()
...
func _draw() -> void:
	...
	var edge := Color(0.08, 0.08, 0.12, 0.95)
	if face_button:
		...
		draw_rect(rect.translated(Vector2(2, 3)), shadow, true)
...
func _gui_input(event: InputEvent) -> void:
	if event is InputEventScreenTouch:
		...
		if touch.pressed and active_touch < 0:
			...
			drag_offset = touch.position - global_position
			if not edit_mode:
		...
		if edit_mode and drag.index == active_touch:
			_set_dragged_position(drag.position - drag_offset)
			accept_event()
	elif event is InputEventMouseButton:
		var mouse_button := event as InputEventMouseButton
		if mouse_button.button_index == MOUSE_BUTTON_LEFT:
			mouse_active = mouse_button.pressed
			drag_offset = mouse_button.position
			if edit_mode:
		...
		_set_dragged_position(global_position + motion.relative)
		accept_event()

func _set_dragged_position(new_global_position: Vector2) -> void:
	var viewport_size := get_viewport_rect().size
	var clamped := Vector2(
		clamp(new_global_position.x, 0.0, max(0.0, viewport_size.x - size.x)),
		clamp(new_global_position.y, 0.0, max(0.0, viewport_size.y - size.y))
	)
